...
function get_workers(){
    ...
    $args = array(
        'exclude' => get_users(array('role' => 'customer'))->pluck('ID'),
    );
    $non_customer_users = get_users($args); // Fetch users who are not customers

    $non_customers = [];

    foreach ($non_customer_users as $user) {
        ...
        $non_customer['role'] = !empty($user->roles) ? $user->roles[0] : null;
